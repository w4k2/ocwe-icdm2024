...
def pairs_metrics_multi(method_names, stream_names, metrics, experiment_names, methods_alias=None, streams_alias=None, metrics_alias=None, treshold=0.5, title=False):
        ...
        for stream_name in stream_names:
            for metric in metrics:
                for experiment_name in experiment_names:
                    try:
                        data[(method_name, stream_name, metric, experiment_name)] = np.genfromtxt(
                            "results/raw_metrics/%s/%s/%s/%s.csv" % (experiment_name, stream_name, metric, method_name))
                    except:
                        print("None is ", method_name,
                              stream_name, metric, experiment_name)
                        data[(method_name, stream_name,
                              metric, experiment_name)] = None
                        print(
                            data[(method_name, stream_name, metric, experiment_name)])
        ...
        for index_j, (metric, metric_a) in enumerate(zip(metrics, metrics_alias)):
            ...
            for stream in tqdm(stream_names, "Rank %s %s" % (experiment_name, metric)):
                ...
                for j, method_2 in enumerate(method_names):
                    ...
                    try:
                        statistic, p_value = stats.ranksums(data[(method_1, stream, metric, experiment_name)], data[(
                            method_2, stream, metric, experiment_name)])
                        if p_value < treshold:
                            if statistic > 0:
            ...
            try:
                axes[index_i, index_j].barh(
                    ma, rank_error, color="black", height=0.9)
                axes[index_i, index_j].barh(
    ...
    if title:
        filename = "results/ranking_plots/ranks_%s_%s" % (
            streams_alias, methods_alias[0])
    else:
